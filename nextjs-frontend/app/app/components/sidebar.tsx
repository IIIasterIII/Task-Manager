...
const Sidebar = () => {
  ...
  useEffect(() => {
    console.log(projectDataCreation, "BUHAHAHA")
    ...
    if (projectDataCreation.parent_id) {
      setOpenFolders(prev => 
        prev.includes(Number(projectDataCreation.parent_id)) 
          ? prev 
          : [...prev, Number(projectDataCreation.parent_id)]
      );
    }
  }, [projectDataCreation])
        ...
        {close ? (
          ...
          >
            ...
            <div className="flex flex-col gap-2 pt-4 border-t border-background-800">
              ...
              <Button title={"Calendar"} func={() => router.push("/app/calendar")}/>
            </div>
          </motion.div>
        ) : (
          <motion.div 
