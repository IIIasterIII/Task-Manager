...
const Page = () => {
  ...
  const handleLogIn = async (e: React.FormEvent) => {
    e.preventDefault()
    const res = await PostLogIn(userData.email, userData.password)
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#050505] px-6 py-12 lg:px-8 relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000] h-[600] bg-[#FE0C46]/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      >
        <div className="text-center mb-10">
          <motion.div 
            initial={{ scale: 0.5 }}
            animate={{ scale: 1 }}
            className="mx-auto w-12 h-12 bg-[#FE0C46] rounded-xl mb-6 shadow-[0_0_32px_0_rgba(254,12,70,0.8)] flex items-center justify-center"
          >
        ...
        <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 p-8 rounded-3xl shadow-2xl">
          <form className="space-y-5">
            <div className="space-y-2">
              <label className="text-[11px] uppercase tracking-widest font-black text-zinc-500 ml-2">Email Address</label>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-[#FE0C46] transition-colors" size={18} />
                <input
                  type="password"
                  placeholder="••••••••"
        ...
        <p className="mt-8 text-center text-sm text-zinc-600">
          ...
          <a className="font-bold text-[#FE0C46] hover:text-white duration-500 cursor-pointer transition-all ml-2" onClick={() => router.push("/auth/register")}>Sign up for free</a>
        </p>
      </motion.div>
    </div>
  )
